...
function SpareOut() {
    ...
    const takeQuantity=(item)=>{
        ...
        {
            newQty=parseInt(item.qty)>=parseInt(myqty)?parseInt(item.qty)-parseInt(myqty):0
        }
        else if(selectedQty==="localQty")
        {
            newLocalQty=parseInt(item.localQty)>=parseInt(myqty)?parseInt(item.localQty)-parseInt(myqty):0
        }
        else if(selectedQty==="servQty")
        {
            newServQty=parseInt(item.servQty)>=parseInt(myqty)?parseInt(item.servQty)-parseInt(myqty):0
        }
        ...
        .then(()=>{
            ...
            set(newHistoryRef, {
                ...
                removed: selectedQty+" : "+myqty, //quantity added
                date: currentDate
            })
            .then(()=>{
                // alert("Successfully updated")
